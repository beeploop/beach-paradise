...
const CottageForm = ({ closeModal, cottageId, state, handleSubmit }: any) => {
    ...
    useEffect(() => {
        if (opened) {
            // fetch(`http://localhost:5000/api/cottage/${cottageId}`)
            fetch(
                `https://beach-reservation.onrender.com/api/cottage/${cottageId}`
            )
